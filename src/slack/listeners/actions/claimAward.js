...
module.exports = (app) => {
  ...
  app.action("award_category_selected", async ({ ack, body, client }) => {
    ...
    try {
      ...
      await client.views.update({
        ...
        view: claimAwardView(categories, selected_category, awards_by_category),
      });
    } catch (error) {
      await client.views.open({
        trigger_id: body.trigger_id,
  ...
  app.action("claim_award", async ({ ack, body, client }) => {
    ...
    try {
      const awardRequest = await awardRequestRepository.createAwardRequest(
        body.user.id,
        awardId
      );
  ...
  blocks: [
    {
      ...
      text: {
        type: "mrkdwn",
        text: "Your request has been submitted! You will get your points as soon as your request is approved.",
      },
    },
  ],
};
